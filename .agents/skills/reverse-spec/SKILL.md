---
name: reverse-spec
description: Bootstrap a living spec by reverse-engineering intent from existing source code. Use when migrating features from prior versions (pelilauta-17, cyan, etc.) into v20, or when speccing brownfield code that has no documentation.
---

# Reverse-Spec Command

Bootstrap a **living spec** by reverse-engineering intent from existing source code.
...
- Convert the GitHub URL to its raw equivalent (e.g., `https://raw.githubusercontent.com/{owner}/{repo}/{branch}/{path}`) and fetch using `curl`.
- **For a directory:** List contents via `gh api repos/{owner}/{repo}/contents/{path}` if `gh` is available, otherwise use the GitHub API via `curl`. Fetch key files (prioritize: schemas/types, main component, index/barrel exports, tests).
