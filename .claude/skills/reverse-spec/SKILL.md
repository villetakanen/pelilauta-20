...
- A **GitHub URL** to a file or directory in a prior-version repo (e.g. `https://github.com/villetakanen/pelilauta-17/tree/main/src/components/client/threads`). Fetch via `raw.githubusercontent.com` / `gh api`.
- A **local path** inside this repo (e.g. `packages/cyan/src/components/cn-card.svelte`). Read directly.
