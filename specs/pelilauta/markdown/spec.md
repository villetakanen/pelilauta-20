...
- **`markdownToPlainText(markdown, maxLength?)`** — markdown-to-plain-text strip for SEO meta tags, notification bodies, and RSS feed `description` fields. v17 lives in `utils/snippetHelpers.ts` as `createPlainSnippet` (regex-based stripping; not a marked call). v20 home: `packages/utils/src/markdownToPlainText.ts`. **Specced** at [`markdown-to-plain-text/spec.md`](markdown-to-plain-text/spec.md) (status: draft, 2026-04-27) — driven by `specs/cyan-ds/layouts/seo/spec.md` Stage-1.
