...
packages/firebase/
  src/
    ...
    client/
      index.ts          → initializeApp (firebase/app), getFirestore, getAuth; re-exports GoogleAuthProvider, signInWithRedirect, getRedirectResult, onAuthStateChanged, signInWithCustomToken
...
- **Firebase Auth redirect handler MUST be proxied through the app's own origin in production.** Without this, `signInWithRedirect` succeeds at Google, returns to `<project>.firebaseapp.com`, and the iframe postMessage handshake back to the app is blocked by browser cross-site storage partitioning — `getRedirectResult` returns null AND `auth.currentUser` is also null, with no visible error. Localhost is exempt; real deploys on separate eTLD+1 origins are not. The fix has three coupled parts:
  1. The hosting target (Netlify) proxies `/__/auth/*` to `https://<project>.firebaseapp.com/__/auth/*` via a `status = 200, force = true` redirect rule in `netlify.toml`. OAuth returns now land on the app's own origin. Skip for deploys whose hostname shares eTLD+1 with a custom Firebase auth subdomain (e.g. `auth.pelilauta.social` + `pelilauta.social` are same-site; no proxy needed).
  2. `PUBLIC_authDomain` is set to the **app's hostname** (e.g. `pelilauta-social-next.netlify.app`) or a same-site custom auth subdomain (e.g. `auth.pelilauta.social` for the eventual `pelilauta.social` deploy), not the default `<project>.firebaseapp.com`. Firebase's client SDK sends the OAuth redirect to whatever `authDomain` says; with the proxy rule in place (or the same-site custom auth subdomain), that redirect resolves first-party and auth state propagates.
  3. Google Cloud Console → **Credentials** → the OAuth 2.0 Client ID used by Firebase → **Authorized redirect URIs** MUST include `https://<authDomain>/__/auth/handler` for every value of `PUBLIC_authDomain` used in any deploy context. Default Firebase-created URIs (e.g. `<project>.firebaseapp.com/__/auth/handler`) are pre-registered at project creation; custom `authDomain` values are not. Missing registration surfaces as `Error 400: redirect_uri_mismatch` from Google before the OAuth flow completes. Propagation after saving can take up to several hours per Google's own warning.
  Removing any of the three reintroduces either the silent-return failure (1 or 2) or the `redirect_uri_mismatch` failure (3). Reference: [Firebase redirect best practices](https://firebase.google.com/docs/auth/web/redirect-best-practices).
...
- [ ] Client entry re-exports `onAuthStateChanged` and `signInWithCustomToken` from `firebase/auth` for use by `AuthHandler` (reconciliation and session recovery — see `specs/pelilauta/session/state-machine.md`).
